...
A sample C23 project built on [samuel-stidham/c-project-skeleton](https://github.com/samuel-stidham/c-project-skeleton). A small interactive calculator stands in for real domain code and shows the skeleton in use: CMake and Ninja drive the build, a thin Makefile provides the daily verbs, warnings are errors from the first compile, and the Check suite tests the same `core` library the app links.

It all works on clone: `make build`, `make test`, and `make lint` before you read a line of the CMake.
...
├── include/         // Public headers (calculator.h)
...
- **Run the app:** `make run` (or `make run BUILD_TYPE=release`), then type a calculation like `3 + 4`
...
The layout is one suite per file: each `test_*.c` exports a suite creator, `tests/suites.h` declares it, and `tests/runner.c` registers it. To add a suite for new code, copy `tests/test_calculator.c`, rename its creator, and register it in both places. The CMake glob picks new files up at the next build.
...
- **Sanitizers:** `make asan`, `make ubsan`, `make tsan`, `make lsan`, or all in turn with `make sanitizers`. Each builds into its own directory, runs the test runner under instrumentation, then runs the app on sample input. Apple clang lacks `-fsanitize=leak`. On macOS, install LLVM with `brew install llvm` and run `make lsan CC="$(brew --prefix llvm)/bin/clang"`.
