# Development Workflow for c-project-sample

This document outlines the recommended workflow for developing, testing, and releasing this project, which is built on [c-project-skeleton](https://github.com/samuel-stidham/c-project-skeleton). The Makefile provides the verbs, and CMake with Ninja does the building.
...
- **Tests:** `make test` builds and runs the Check suite through ctest with failure output shown. One suite per file: copy `tests/test_calculator.c`, rename its creator, declare it in `tests/suites.h`, and register it in `tests/runner.c`.
...
- `make asan`, `make ubsan`, `make tsan`, and `make lsan` each build instrumented binaries in their own directory, run the tests, then run the app on sample input. `make sanitizers` runs all four.
